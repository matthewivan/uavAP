...
MicroSimulator::simulate(Duration duration)
{
    ...
    while (now_ <= endSim)
    {
        ...
        {
            TimePoint untilTime = startTimeReal + Duration(static_cast<Duration::rep>(std::chrono::duration_cast<Duration>(events_.begin()->first - startTimeSim).count() * realTimeFactor_));
